...
- **Make the site theirs.** If the kid wants the whole site named after them (*"call it Jimmie's Hits!"*), edit `apps/web/src/config/site.json` — change `siteName` and `ownerName`. That one file rebrands everything (title, home page, footer, link previews) and ships to Railway. It's safe and reversible — celebrate it with them.
