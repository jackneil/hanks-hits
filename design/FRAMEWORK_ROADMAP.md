...
1. **`my-creations` skill** — a private, local gallery of the kid's OWN games (the #1 missing stickiness surface; cheap, zero exposure risk). ✅ SHIPPED (`.claude/skills/my-creations/`)
2. **`remix-a-game` skill** — copy an existing working game into a new id and re-theme it; make "start from one you love" the default, never a blank page (most-cited mechanism in the research; >25% of Scratch projects are remixes). ✅ SHIPPED (`.claude/skills/remix-a-game/`)
3. **`oops-go-back` undo + pre-edit checkpoint in `change-a-game`** — kids can fearlessly try a change and revert (forgiving/undo is a core engagement hook). ✅ SHIPPED (`.claude/skills/oops-go-back/`)
...
5. **CLAUDE.md "talk-to-a-non-reader" block + read-aloud option** — branch on Player Card age; 6–8yos are emergent readers and text-only is an accessibility fail. ✅ SHIPPED 2026-09-05 (every game and playable app opens through the shared `GameStartOverlay`; a 🔊 "Read it to me" button on the start screen, pause menu, restart dialog, and home cards reads the words aloud with the browser speech API; CLAUDE.md has the non-reader block)
6. **In-app "My Games" shelf** — separate the kid's creations from the 24 built-ins on the home page, with personal-best stats. ✅ SHIPPED 2026-07-17 (`madeByKid` flows through both metadata channels; home shelf + profile "Games I Made"; see `design/MY_GAMES_SHELF.html`)
...
| ✅ **"talk-to-a-non-reader" behavior** — SHIPPED 2026-09-05: age ≤8 (or on request): one short sentence at a 1st–3rd-grade level, emoji/either-or over paragraphs, and a 🔊 read-aloud button (browser TTS via `useReadAloud`) on every start screen, pause menu, restart dialog, and home card. No important instruction text-only. | claude-behavior + app-mechanism | small | CLAUDE.md (non-reader subsection); play-my-game; `shared/hooks/useReadAloud.ts`, `shared/components/ReadAloudButton.tsx` |
...
| ✅ **In-app "My Games" shelf** — SHIPPED 2026-07-17 via the `madeByKid` metadata marker (not a localStorage list — the marker travels with the game folder, so it survives clones): top home-page shelf with personal-best stats from Zustand progress + profile "Games I Made". | app-mechanism | medium | `HomeClient.tsx` + `game-registry.ts` + `gameMetadata.generated.ts`; see `design/MY_GAMES_SHELF.html` |
