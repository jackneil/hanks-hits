...
import { HextrisGameShell } from "../HextrisGameShell";
import { useHextrisStore } from "../lib/store";

describe("HextrisGameShell pause wiring", () => {
  beforeEach(() => {
  ...
  it("wires the shell's ESC/pause to the store and keeps the two in sync", () => {
    act(() => {
      useHextrisStore.setState({ status: "playing" });
    });
    render(<HextrisGameShell />);
    ...
    expect(useHextrisStore.getState().status).toBe("paused");
