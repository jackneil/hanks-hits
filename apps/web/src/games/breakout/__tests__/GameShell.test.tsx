...
import { BreakoutGameShell } from "../BreakoutGameShell";
import { useBreakoutStore } from "../lib/store";

describe("BreakoutGameShell pause wiring", () => {
  beforeEach(() => {
  ...
  it("wires the shell's ESC/pause to the store and keeps the two in sync", () => {
    act(() => {
      useBreakoutStore.setState({ status: "playing" });
    });
    render(<BreakoutGameShell />);
