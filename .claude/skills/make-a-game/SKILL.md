...
Quick check: look in `apps/web/src/games/` and `apps/web/src/apps/` for a matching game. If it already exists and works, **don't re-scaffold a duplicate** (you'll clobber another game's island) — switch to **change-a-game**, or build a genuinely different twist under a NEW id. Re-using an existing game still needs its design note (Step 3) if one is missing.
