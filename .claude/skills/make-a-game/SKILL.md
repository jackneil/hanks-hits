...
③ `apps/web/src/shared/lib/gameStatExtractor.ts` — add a `case "my-game":` with `primaryStat` + a couple of `secondaryStats`, or the profile page silently falls through to a generic high-score.
...
⑥ `src/games/my-game/metadata.ts` — `export const metadata: GameMetadata` (import type from `@/shared/lib/game-registry`). **Only `id`, `name`, `emoji`, `category` are required** (`description`, `hidden` are optional). The home page finds your game by **regex-scanning this file at runtime**, so every value MUST be a **plain string literal** (`"donut-catch"`) — never a computed value, imported constant, or template string, or the game is silently skipped. **Do not add a `color` field** — card color is auto-derived from `category`. **Set `madeByKid: true` (plain literal)** — this is what puts the game on the home page's **"🌟 My Games" shelf** and the profile's **"🛠️ Games I Made"** section (and the my-creations chat skill). Forgetting it is a fully-green silent failure: everything compiles and the game shows in its category, but the kid's own shelf never lists it. The home shelf picks it up instantly in dev; the profile section refreshes on the next `pnpm build`.
