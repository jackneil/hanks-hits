---
description: "Parallel UX checks — standalone (generated 2026-07-01; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Stack**: Next.js 16 + React 19 · React Three Fiber + Rapier (3D canvas games) · Tailwind v4 + DaisyUI
- **Browser Tool**: Playwright MCP
- **Dev Server Port**: 3000 (`cd apps/web && pnpm dev` — Next.js default)
...
## Credential Hints
none found — auth is next-auth **credentials** (email + password). Create a test user via `/signup`, or ask the user for login creds if a logged-in flow is under test. (`PGPASSWORD` in env is Postgres infra, not a UI login.)

## UX Focus Areas
- **Audience is kids 6–14** — emphasize big tap targets (≥ 44px), bright/forgiving UI, celebratory feedback (confetti/sound on collect), and no punishing failure states. Gut check every flow against "would a 9-year-old on a phone get this instantly?"
- **Tailwind v4 (mobile-first)**: verify responsive breakpoints at 375px / 768px / 1280px — most kids are on phones/tablets, so mobile is primary, not an afterthought
- **3D / driving games**: verify touch pedals + tilt steering on mobile and keyboard on desktop; confirm auto-recovery (flipped truck rights itself) and that the big horn/action buttons are reachable
- **Next.js App Router**: test client/server boundaries and the `ssr:false` dynamic game imports; watch for hydration mismatches
- **Home grid + discoverability**: confirm every game/app carrying a `metadata.ts` appears on the home grid and is reachable in minimal clicks; new games should be easy to find
- **Nav/layout changes**: emphasize the Discoverability aspect across all agents
