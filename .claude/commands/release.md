...
- **Project**: hanks-hits (Hank's Hits — kid-driven game-building platform; pnpm monorepo `apps/web` + `packages/db`)
- **Base Branch**: master
- **Shipping Model**: pr-to-main-deploy (Railway auto-deploys every push/merge to `master`; nothing is published to a package index — `apps/web` is a private app, not an npm package)
- **Version Source**: none — a release does NOT bump a version. (`apps/web/package.json` carries `0.1.0` but it is never bumped for a release and nothing reads it as a release version — do not touch it.)
- **Gate Commands**: the repo's ONLY gate is the local CI chain (there is no GitHub CI). Run from the REPO ROOT, in order, STOP on the first failure — this mirrors `.githooks/pre-push`, which also enforces it on push:
