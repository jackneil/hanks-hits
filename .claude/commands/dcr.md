...
- The ROM/asset proxy (`/api/roms/[...path]`) is coaxed into fetching an attacker-controlled or internal URL (SSRF), or is hammered by an unauthenticated caller to drive unbounded upstream CDN fetches (amplification — it is rate-limited per IP, verify that holds), or a non-owner forges a write to another user's progress/profile/leaderboard.
