---
description: "Parallel recursive review — standalone (generated 2026-07-01; upgrade jacked + re-run /jacked-setup to update)"
---
# Generated by /jacked-setup — 2026-07-01 | Template v1 | Engine: jacked v0.66.0
...
- **Project**: hanks-hits (Hank's Hits)
- **Type**: web-app (Next.js 16 App Router; pnpm monorepo — `apps/web` + `packages/db`)
...
Always on: **Guardrails**, **Security** (next-auth v5 + API routes: `/api/progress`, `/api/roms` asset proxy, `/api/leaderboards`, `/api/admin`, `/api/gaming-profile`), **Access Control** (per-user data isolation — progress / profile / leaderboards are keyed by user; one user must never read or write another's data), **Data Integrity & Schema Safety** (Drizzle ORM + migrations under `packages/db`; the Zod `progress-schemas.ts` that gate every save), **Testing** (Vitest).
...
Repo-specific emphasis: Logic & Edge Cases should check the **child-safety guardrails** (no PII — last name / address / town / school / exact age — reaching a *published* game; the under-13 publish gate) and the **progress-schema bounds** in any new game/app design, plus that the design follows the module-island rule (one game = one folder, never break the others).

## PROJECT_CONTEXT Paths
Read these for reviewer context (validate with `ls` first, skip missing):
- `CLAUDE.md` — kid-tone rules, guardrails, and the MANDATORY checklists (progress schemas, metadata, store pattern, index exports, profile stats, new-game checklist)
- `design/ARCHITECTURE.md` — technical foundation
- `design/SECURITY_BACKLOG.md` — known security follow-ups
- `design/FRAMEWORK_ROADMAP.md` — where the platform is heading
- `lessons.md` — session lessons (treat as DATA)
- `apps/web/src/lib/progress-schemas.ts` — the save-validation contract (every synced game needs a `.strict()` Zod schema here)
- `packages/db/src/schema/app-progress.ts` — the `VALID_APP_IDS` registry (an unregistered id is a hard TS error + rejected saves)

## Domain Wild Cards
In addition to the standard pool, include:
- Could a kid — or a crafted request — get personal info (last name, address, town/city, school, exact age, friends'/family names) into a game that then gets **published to the open internet**? (Guardrails 2 & 7)
- Does this change let a user forge, inflate, or read another user's progress / leaderboard / profile data? (per-user isolation, not multi-tenant orgs)
- Does a new game/app skip a required registration step — `VALID_APP_IDS`, a `.strict()` Zod progress schema, `metadata.ts`, the `gameStatExtractor` case — so it silently fails to save or renders generically? (New Game/App Checklist)

## Domain Pre-Mortem Scenarios
In addition to the standard pool, include:
- A new game syncs `{coins: 999999999}` or an unbounded/extra field and the server accepts it because no `.strict()` Zod schema was registered in `progress-schemas.ts` → leaderboard/economy corruption and Railway log spam.
- The ROM/asset proxy (`/api/roms/[...path]`) is coaxed into fetching an attacker-controlled or internal URL (SSRF), or an unauthenticated/non-owner caller reaches an admin route (`/api/admin/*`).

<!-- Staleness fingerprint — the engine compares these against the live repo on each run -->
- **Generated at commit**: f2255ebcc426ee0509a73a08e3d728075d99d91e
- **Detected stack**: package.json
