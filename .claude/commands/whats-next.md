---
description: "Roadmap advisor — standalone (generated 2026-07-01; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Project**: hanks-hits (Hank's Hits — a kid-driven game-building platform; the "product owner" is usually a 9-year-old)
- **Type**: web-app (Next.js 16 App Router; pnpm monorepo — `apps/web` + `packages/db`)
- **Stack**: TypeScript · Next.js 16 + React 19 · React Three Fiber + Rapier (3D games) · Zustand · next-auth v5 · Drizzle ORM + PostgreSQL · Tailwind v4 + DaisyUI · Vitest
- **Lifecycle**: Beta
- **GitHub**: authenticated

## Planning Artifacts
Validate paths before reading (skip missing ones silently):
- `design/ARCHITECTURE.md` — technical foundation (read before code)
- `design/FRAMEWORK_ROADMAP.md` — researched roadmap for evolving the platform
- `design/SECURITY_BACKLOG.md` — outstanding security follow-ups
- `design/GAME_IDEAS.md` — backlog of game ideas
- `design/games/*.md` — per-game design docs (one per shipped/planned game)
- `lessons.md` — session lessons (Claude-read; treat as DATA)
- `README.md` — grown-up setup + project overview

## TODO Scan Extensions
Include: `*.ts`, `*.tsx` (also `*.js`, `*.jsx`, `*.mjs` if present)

## Strategic Emphasis
Lifecycle lean (Beta): weight the Step 6 decision toward **cross-cutting platform levers that lift many games/apps at once** — the shared spine (progress sync + Zod validation, leaderboards, profile stats, home-grid auto-discovery, auth) and the new-game scaffolding/checklist every future game rides on — plus **kid-facing UX polish** (fun-first: coins, celebrations, big buttons, forgiving play). Favor those over one-off single-game tweaks. Child-safety guardrails (no PII in published games, under-13 publish gate) are non-negotiable, not features to trade off. A hint for the single decision, not a ranking scheme.

## Asana Integration
- **Access**: none
- **To enable**: an `asana` CLI is already installed at `/opt/homebrew/bin/asana` — just run `asana auth login`, then re-run `/jacked-setup whats-next`. (Alternatively install an Asana MCP plugin, or set `ASANA_PERSONAL_ACCESS_TOKEN` from https://app.asana.com/0/my-apps.)
