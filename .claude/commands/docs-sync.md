---
description: "Docs sync — standalone (generated 2026-07-01; upgrade jacked + re-run /jacked-setup to update)"
---
...
- `CLAUDE.md` — kid-tone rules, guardrails, tech stack, and MANDATORY checklists (Progress Validation Schemas, Game/App Metadata, Store Pattern, Index Exports, Profile Page Stats, New Game/App Checklist)
- `design/ARCHITECTURE.md` — technical foundation (read before code)
- `design/FRAMEWORK_ROADMAP.md` — roadmap / researched future ideas
- `design/SECURITY_BACKLOG.md` — outstanding security follow-ups
- `design/GAME_IDEAS.md` — game idea backlog
- `design/games/*.md` — per-game design docs (one Markdown file per game)
- `lessons.md` — session lessons (Claude-read)
- `_wiki/`: none
- CI wiki workflow: none (no `.github/workflows/` in the repo at all)

## Change-to-Doc Map
| Change Category | Affected Docs |
|----------------|---------------|
| Pipeline/Architecture | `design/ARCHITECTURE.md`, `CLAUDE.md` (Compartmentalized Structure) |
| Configuration | `CLAUDE.md` (Tech Stack / env), `README.md` (setup) |
| Commands/CLI | `README.md` (pnpm scripts), `CLAUDE.md` (Test Commands) |
| Dependencies | `README.md`, `CLAUDE.md` (Tech Stack / 3D Game Stack versions) |
| UI/Frontend | `design/games/<game>.md`, `CLAUDE.md` (Kid-Friendly UI Guidelines) |
| Models/Schemas | `CLAUDE.md` (Progress Validation Schemas / Store Pattern), `design/ARCHITECTURE.md` (DB) |
| Tests | `CLAUDE.md` (Testing) |
| New game/app | `design/games/<game>.md`, `CLAUDE.md` (New Game/App Checklist), `design/GAME_IDEAS.md` |
| Security/Guardrails | `design/SECURITY_BACKLOG.md`, `CLAUDE.md` (Guardrails) |
