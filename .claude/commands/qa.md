---
description: "Browser QA — standalone (generated 2026-07-01; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Tailwind v4 + DaisyUI**: check responsive classes at mobile/tablet breakpoints; enforce kid-UI rules — buttons ≥ 44×44px, bright high-contrast colors, no tiny tap targets
- **Next.js 16 App Router**: 3D games load via `dynamic(() => import(...), { ssr: false })` — verify no SSR/hydration errors and the canvas mounts client-side only
- **R3F / Rapier games**: watch the browser console for WebGL/physics warnings; confirm touch pedals + tilt steering on mobile (driving games) and keyboard (WASD/arrows) on desktop; forgiving physics (truck auto-recovers)
- **Progress/save**: confirm score/progress persists and survives a full reload (localStorage + cloud sync); no console errors from the progress API
